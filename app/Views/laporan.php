...
<script>
    ...
    function cetakPDF(jenis) {
        ...
        doc.text('Laporan Penjualan', doc.internal.pageSize.getWidth() / 2, 16, { align: 'center' });
        doc.setFontSize(10);
        
        if (jenis === 'hari') {
            doc.text('Laporan Penjualan Harian', doc.internal.pageSize.getWidth() / 2, 22, { align: 'center' });
            doc.text('Periode ' + formatTanggal(tanggalMulai), doc.internal.pageSize.getWidth() / 2, 28, { align: 'center' });
        } else if (jenis === 'bulan') {
            var bulanTahun = tanggalMulai.toLocaleString('id-ID', { month: 'long', year: 'numeric' });
            doc.text('Laporan Penjualan Bulanan', doc.internal.pageSize.getWidth() / 2, 22, { align: 'center' });
            doc.text('Periode ' + bulanTahun, doc.internal.pageSize.getWidth() / 2, 28, { align: 'center' });
        } else if (jenis === 'tahun') {
            var tahun = tanggalMulai.getFullYear();
            doc.text('Laporan Penjualan Tahunan', doc.internal.pageSize.getWidth() / 2, 22, { align: 'center' });
            doc.text('Periode Tahun ' + tahun, doc.internal.pageSize.getWidth() / 2, 28, { align: 'center' });
        } else {
            doc.text('Periode ' + formatTanggal(tanggalMulai) + ' - ' + formatTanggal(tanggalSelesai), doc.internal.pageSize.getWidth() / 2, 28, { align: 'center' });
        }
